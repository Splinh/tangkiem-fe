...
│   │   ├── page.tsx            # Trang chủ
│   │   ├── truyen/
│   │   │   ├── page.tsx        # Danh sách truyện (có filter, pagination)
│   │   │   └── [slug]/
...
│   │   │   └── [slug]/page.tsx # Truyện theo thể loại
...
Browser request → Next.js Server (SSR)
                      ↓
              serverFetch() → API (X-Secret-Key header)
                      ↓
              Render HTML → Browser
...
# 2. Nén (bỏ node_modules, .next/cache)
# Upload .next + public + package*.json + config files
...
export async function getNewData(params?: Record<string, string>) {
  ...
  return serverFetch<{ data: NewType[] }>(`/new-endpoint${query}`);
}
```
...
2. CSS dùng CSS Modules hoặc inline styles
3. Tái sử dụng các class CSS global (`.card`, `.badge`, `.story-card`, etc.)
...
| `.story-card`                                                   | Story thumbnail card    |
